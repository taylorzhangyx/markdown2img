...
export async function renderCoverPage(page: Page, meta: ArticleMeta, outputDir: string, pageNumber = 1): Promise<string> {
  ...
  const coverHtml = [
    ...
    `    .cover { position: relative; width: ${LAYOUT.PAGE_WIDTH}px; height: ${LAYOUT.PAGE_HEIGHT}px; padding: 76px 68px 80px; background: linear-gradient(180deg, #F7F4EF 0%, #F4F1EA 100%); }`,
    '    .cover::before { content: ""; position: absolute; inset: 28px; border: 1px solid rgba(95, 86, 76, 0.12); border-radius: 32px; pointer-events: none; }',
    '    .content { position: relative; z-index: 1; display: flex; flex-direction: column; height: 100%; }',
    '    .summary-stage { flex: 1 1 auto; display: flex; align-items: center; justify-content: center; padding: 0 0 84px; }',
    '    .summary-frame { position: relative; width: min(936px, calc(100% - 4px)); max-height: 100%; height: 100%; display: flex; align-items: center; justify-content: center; }',
    '    .summary { --summary-font-size: 52px; position: relative; margin: 0; width: 100%; padding-left: 46px; color: #4C3E2F; font-family: "CoverNotoSerifSC", "Noto Serif SC", "Source Han Serif SC", "Source Han Serif CN", "Songti SC", "STSong", serif; font-size: var(--summary-font-size); font-weight: 595; line-height: 1.62; letter-spacing: 0; text-align: justify; text-justify: inter-ideograph; }',
    '    .summary::before { content: "“"; position: absolute; left: -18px; top: -28px; color: rgba(184, 132, 71, 0.9); font-size: clamp(82px, calc(var(--summary-font-size) * 1.88), 122px); line-height: 0.8; }',
    '    .identity { position: absolute; left: 0; bottom: 0; display: flex; align-items: center; gap: 19px; color: #5C4C3C; }',
    '    .avatar { width: 66px; height: 66px; border-radius: 999px; object-fit: cover; background: #ECE5DA; border: 1px solid rgba(95, 86, 76, 0.12); }',
    '    .identity-copy { display: flex; flex-direction: column; gap: 5px; min-width: 0; }',
    '    .author { font-family: "CoverNotoSerifSC", "Noto Serif SC", "Source Han Serif SC", "Source Han Serif CN", "Songti SC", "STSong", serif; font-size: 27px; line-height: 1.2; font-weight: 620; color: #584633; }',
    '    .date { font-family: "PingFang SC", "Hiragino Sans GB", "Noto Sans CJK SC", sans-serif; font-size: 19px; line-height: 1.2; color: #90725A; letter-spacing: 0.03em; }',
    '  </style>',
    '  <script>',
    '    function fitSummary() {',
